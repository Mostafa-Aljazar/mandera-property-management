...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  if (!tenant) return apiError("not_found", "المستأجر غير موجود", 404);
  ...
  const uploaded = await uploadOwnerImage(supabase, "tenant-documents", path, file);
  ...
  const { data, error } = await supabase
    .from("tenants")
    .update({ id_document_url: uploaded.url })
    .eq("id", id)
    .eq("owner_id", ownerId)
    .select("*")
    .single();
